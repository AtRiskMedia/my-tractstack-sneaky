...
const StyleCodeHookPanel = ({
  node,
  availableCodeHooks = [],
}: ExtendedBasePanelProps) => {
  ...
  const handleOptionKeyChange = useCallback((index: number, newKey: string) => {
    setLocalOptions((prev) =>
      prev.map((opt, idx) =>
        idx === index ? { ...opt, key: newKey, isDirty: true } : opt
      )
    );
  }, []);
    ...
    (index: number) => {
      ...
      setLocalOptions((prev) => {
        const updated = prev.map((opt, idx) =>
          idx === index ? { ...opt, isDirty: false } : opt
        );
  ...
  const handleOptionValueChange = useCallback(
    (index: number, newValue: string) => {
      ...
      setLocalOptions((prev) =>
        prev.map((opt, idx) =>
          idx === index ? { ...opt, value: newValue, isDirty: true } : opt
        )
      );
    },
    [isInitialized]
  ...
  const toggleBooleanOption = useCallback(
    (index: number) => {
      ...
      setLocalOptions((prev) => {
        const updated = prev.map((opt, idx) =>
          idx === index
            ? { ...opt, value: opt.value === 'true' ? 'false' : 'true' }
            : opt
        );
        const payload = updated.reduce(
          (acc, { key, value }) => {
          ...
          {!isValidCodeHook && localTarget && (
            <div className="mt-2 flex items-center text-amber-500">
              ...
              <span className="text-sm">
                Warning: Selected code hook is not available
              </span>
            </div>
          )}
        </div>
