...
const StyleWidgetUpdatePanel = ({
  ...
  useEffect(() => {
    ...
    const hasOverride =
      targetNode.overrideClasses?.mobile?.[className] !== undefined;
  ...
  }, [node, parentNode, className, childId, isWidget]);
  ...
  useEffect(() => {
    ...
    if (isOverridden) {
      ...
      markdownNode.defaultClasses[node.tagName][pendingUpdate.viewport][
        className
      ] = pendingUpdate.value;
...
export default StyleWidgetUpdatePanel;
