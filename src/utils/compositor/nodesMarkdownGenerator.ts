...
export const hasWidgetChildren = (
  nodeId: string,
  ctx: NodesContext
): boolean => {
...
export class MarkdownGenerator {
  ...
  markdownFragmentToMarkdown(nodeId: string): string {
    const node = this._ctx.allNodes
      .get()
      .get(nodeId) as MarkdownPaneFragmentNode;
  ...
  flatNodeToMarkdown(nodeId: string): string {
    ...
    switch (type) {
      ...
      case 'li': {
        if (
          hasWidgetChildren(nodeId, this._ctx) ||
          hasImgChildren(nodeId, this._ctx)
        ) {
          return childrenMarkdown;
        }
        this._listIdx++;
  ...
  markdownToFlatNodes(markdown: string, parentId: string): FlatNode[] {
    ...
    while (i < lines.length) {
      ...
      if ((match = /^(\d+)\.\s+(#{1,6}\s+)?(.*)$/.exec(line))) {
        ...
        const tagNameCustom = isHeading
          ? `h${headingMarks.trim().length}`
          : 'p';
